...
try:
    ...
    if not os.path.exists(model_path):
        st.error(f"Error: The model file '{model_path}' was not found. Check the file path.")
    else:
        model = joblib.load(model_path)
        st.success("Model loaded successfully!")
except joblib.MemoryError:  # Handling memory-related errors
    st.error("Error: Insufficient memory to load the model.")
